...
int main() {
    ...
    float norm = std::sqrt(thrust::transform_reduce(
        ...
        [] __host__ __device__ (float x) { return x * x; }, // Lambda para o quadrado
    ...
    thrust::transform(
        d_vec.begin(), d_vec.end(), d_vec.begin(),
        [norm] __host__ __device__ (float x) { return x / norm; } // Lambda para normalizar
    );
